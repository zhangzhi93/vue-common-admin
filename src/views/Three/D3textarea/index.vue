<template>
  ...
  <a-button @click="updateTexture" style="position: absolute; left: 10px; top: 10px">更新</a-button>
</template>
...
function updateTexture() {
  i = 0;
  let datac = new Uint8Array(size * size * size);
      ...
      for (let x = 0; x < size; x++) {
        const d =
          ...
          vector
            ...
            .divideScalar(size)
            .length();
        datac[i] = (128 * Math.random() + 128 * perlin.noise((x * scale) / 1.5, y * scale, (z * scale) / 1.5)) * d * d;
        i++;
      }
    }
  }
